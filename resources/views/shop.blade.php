    ...
    <div class="booking-layout">
        ...
        <div class="panel table-selector"><div class="eyebrow">Zone {{ $selectedZone?->code }}</div><h2>เลือกโต๊ะ {{ $selectedZone?->code }}01–{{ $selectedZone?->code }}60</h2><p class="help">แถว 1 อยู่ใกล้เวที · โต๊ะละ 8 ท่าน · คลิกหมายเลขเพื่อดูรายละเอียด</p>
            ...
            <div id="reserve" class="reserve-panel">@if($selectedTable && $selectedZone->is_active && $selectedTable->is_active && !$selectedTable->held_by_order_id && !$selectedTable->sold_by_order_id)
                ...
                @auth <form class="form" method="post" action="{{ route('orders.store') }}">@csrf<input type="hidden" name="kind" value="table"><input type="hidden" name="resource_id" value="{{ $selectedTable->id }}"><input type="hidden" name="quantity" value="1"><input type="hidden" name="client_key" value="{{ (string) Str::uuid() }}"><div class="field"><label for="dietary_notes">อาหารที่แพ้ / ไม่รับประทาน (ถ้ามี)</label><textarea class="input" id="dietary_notes" name="dietary_notes" rows="2" maxlength="1000" placeholder="เช่น แพ้กุ้ง ไม่รับประทานเนื้อวัว"></textarea></div><button class="btn" type="submit">จองโต๊ะนี้</button></form>
